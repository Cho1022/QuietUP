...
QuietUp은 대면 갈등 없이 층간소음 상황을 알리고, 당사자와 관리 주체가 대응 이력을 확인할 수 있도록 만드는 것을 목표로 합니다. 현재 저장소에는 초기 Android MVP가 보존되어 있으며, 서버에는 인증·거주 인증, 공동체 경계 기반 익명 소음 알림·정형 응답·대응 이력과 사건 참여자 제한형 REST 채팅이 구현되어 있습니다.
...
목표 아키텍처의 서버 애플리케이션과 로컬 MySQL 기반은 구현됐지만 AWS 배포는 아직 구현되지 않았습니다. 현재는 서버 인증 API, 거주 인증 도메인, 익명 소음 알림·정형 응답·대응 이력과 제한형 REST 채팅 API까지 구성되어 있으며 Android REST 연동은 아직 구현하지 않았습니다. 다음 원칙을 기준으로 전환합니다.
...
- `REQUEST_CHAT` 최초 응답자와 알림 발신자만 참여하는 제한형 익명 채팅방, REST 메시지 커서 조회와 멱등 종료를 구현했습니다.
- 같은 대상 세대의 다른 거주자도 채팅에는 접근할 수 없으며 참여자는 역할 라벨로만 표현합니다.
- Testcontainers가 실제 MySQL 8.4에서 인증·거주·익명 소음 알림·제한형 채팅 API와 Flyway 스키마를 검증합니다.
...
인증 API 계약과 보안 원칙은 [인증 API 문서](docs/api/authentication.md)에서, 거주 인증 정책은 [거주 인증 도메인 문서](docs/domain/residence-verification.md)에서, 알림의 공동체 경계는 [익명 소음 알림 도메인 문서](docs/domain/anonymous-noise-alert.md)에서, 채팅 참여자와 역할 기반 비식별 원칙은 [제한형 익명 채팅 도메인 문서](docs/domain/restricted-anonymous-chat.md)에서 확인할 수 있습니다.
...
7. 익명 소음 알림·대응 이력과 사건 참여자 제한형 채팅 API 구현
